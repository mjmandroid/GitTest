...
public class PhotoPickerActivity extends Activity implements PhotoAdapter.PhotoClickCallBack {
	...
	private void getPhotosSuccess() {
		...
		final List<PhotoFolder> floders = new ArrayList<PhotoFolder>();
		for (String key : keys) {
			if (ALL_PHOTO.equals(key)) {
				PhotoFolder floder = mFloderMap.get(key);
	 ...
	 * @param photoFloder
	 */
	public void selectFloder(PhotoFolder photoFloder) {
...
//			}
			...
			mTmpFile = new File(cameraPath, "photo_temp.jpg");
			Uri photoURI = FileProvider.getUriForFile(this, "com.gaoshoubang.fileProvider", mTmpFile);
