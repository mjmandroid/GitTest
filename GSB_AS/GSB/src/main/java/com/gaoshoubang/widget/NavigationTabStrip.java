...
public class NavigationTabStrip extends View implements ViewPager.OnPageChangeListener {
	...
	private boolean isIntervalLine = true;// 间隔线
	private float stripLength = 0.5f;// 0~1f下划线的长度
	...
	// Set on tab bar selected index listener where you can trigger action
	// onStart or onEnd
	...
	public void setTabIndex(int tabIndex, boolean isForce) {
		...
		// Set startX and endX for animation, where we animate two sides of rect
	...
	public boolean isIntervalLine() {
		return isIntervalLine;
	}

	public void setIntervalLine(boolean isIntervalLine) {
		this.isIntervalLine = isIntervalLine;
	}
	...
	private void updateIndicatorPosition(final float fraction) {
		...
		mStripLeft = mStartStripX + (mResizeInterpolator.getResizeInterpolation(fraction, mIsResizeIn) * (mEndStripX - mStartStripX));
	...
	protected void onMeasure(final int widthMeasureSpec, final int heightMeasureSpec) {
		...
		if (isInEditMode() || !mIsViewPagerMode) {
			...
			mStartStripX = (mIndex * mTabSize) + (mStripType == StripType.POINT ? mTabSize * 0.5F : 0.0F);
			mEndStripX = mStartStripX;
			updateIndicatorPosition(MAX_FRACTION);
		}
	}
	...
	protected void onDraw(final Canvas canvas) {
		...
		if (stripLength != 0 && stripLength < 1f) {
			float l = (mStripRight - (mStripType == StripType.POINT ? mStripWeight * 0.5F : 0.0F)) - (mStripLeft - (mStripType == StripType.POINT ? mStripWeight * 0.5F : 0.0F));
			lengthOffset = (l - (l * stripLength)) / 2;
		}

		// Set bound of strip
		mStripBounds.set(lengthOffset + mStripLeft - (mStripType == StripType.POINT ? mStripWeight * 0.5F : 0.0F), mStripGravity == StripGravity.BOTTOM ? mBounds.height() - mStripWeight : 0.0F,
				mStripRight - (mStripType == StripType.POINT ? mStripWeight * 0.5F : 0.0F) - lengthOffset, mStripGravity == StripGravity.BOTTOM ? mBounds.height() : mStripWeight);
		// Draw strip
		if (mCornersRadius == 0) {
		...
		for (int i = 0; i < mTitles.length; i++) {
			...
			// There is a strange logic of ViewPager onPageScrolled method, so
			// it is
			...
			// 间隔线
			if (isIntervalLine && i != 0 && i != mTitles.length) {
				float height = mBounds.height() - mStripWeight;
				canvas.drawLine(mBounds.width() / mTitles.length * i, height - height / 3, mBounds.width() / mTitles.length * i, height - height / 3 * 2, linePaint);
			}
		}
	...
	private void updateCurrentTitle(final float interpolation) {
		mTitlePaint.setColor(Integer.valueOf(mColorEvaluator.evaluate(interpolation, mInactiveColor, mActiveColor).toString()));
	}

	// Method to transform last fraction of NTS and position
	private void updateLastTitle(final float lastInterpolation) {
		mTitlePaint.setColor(Integer.valueOf(mColorEvaluator.evaluate(lastInterpolation, mActiveColor, mInactiveColor).toString()));
	}
	...
	public void onPageSelected(final int position) {
		// This method is empty, because we call onPageSelected() when scroll
		// state is idle
	}
	...
	// Resize interpolator to create smooth effect on strip according to
	// inspiration design
