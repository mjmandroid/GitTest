...
public class MonthView extends LinearLayout {
	...
	public static MonthView create(ViewGroup parent, LayoutInflater inflater, DateFormat weekdayNameFormat, Listener listener, Calendar today, int dividerColor, int dayBackgroundResId,
	                               int dayTextColorResId, int headerTextColor, Locale locale, DayViewAdapter adapter) {
		return create(parent, inflater, listener, today, dividerColor, dayBackgroundResId, dayTextColorResId, null, locale, adapter);
	}

	public static MonthView create(ViewGroup parent, LayoutInflater inflater, Listener listener, Calendar today, int dividerColor, int dayBackgroundResId, int dayTextColorResId,
		...
		// final CalendarRowView headerRow = (CalendarRowView)
	...
	// private static int getDayOfWeek(int firstDayOfWeek, int offset, boolean
	// isRtl) {
	...
	boolean isHasToday = false;

	@SuppressLint("NewApi")
	public void init(MonthDescriptor month, List<List<MonthCellDescriptor>> cells, boolean displayOnly, Typeface dateTypeface) {
		...
		for (int i = 0; i < 6; i++) {
			...
			if (i < numRows) {
				...
				for (int c = 0; c < week.size(); c++) {
					...
					if (cell.isSelectable() && !cell.isSelected()) {
						if (cell.getTodayDate().getTime() > cell.getDate().getTime()) {
							cellView.getDayOfMonthTextView().setTextColor(Color.parseColor("#a4a4a4"));
						}
						else {
							cellView.getDayOfMonthTextView().setTextColor(Color.parseColor("#434343"));
						}
					}
					...
					if (cell.getTodayDate().getTime() > cell.getDate().getTime() && !cell.isToday()) {
						cell.setSelected(false);
						cellView.setEnabled(false);
						cellView.getDayOfMonthTextView().setBackgroundColor(Color.parseColor("#00000000"));
						cellView.getDayOfMonthTextView().setTextColor(Color.parseColor("#434343"));
					}

					if (cell.isToday() && cell.isSelectable()) {
						isHasToday = true;
						...
						if (cell.isSelected()) {
							...
							cellView.getDayOfMonthTextView().setTextColor(Color.parseColor("#434343"));
							cellView.getDayOfMonthTextView().setBackgroundResource(R.drawable.bg_circle_today);
						}
					}
					else {
						cellView.getDayOfMonthTextView().setBackgroundColor(Color.parseColor("#00000000"));
