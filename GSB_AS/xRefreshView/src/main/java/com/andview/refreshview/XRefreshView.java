...
public class XRefreshView extends LinearLayout {
	...
	private void addFooterView(OnGlobalLayoutListener listener) {
		...
		if (needAddFooterView()) {
			// Log.d("CustomView", "add footView" + ";mHeaderViewHeight=" +
			// mHeaderViewHeight);
	...
	protected void onLayout(boolean changed, int l, int t2, int r, int b) {
		...
		mFootHeight = mFooterView.getFooterHeight();
		int childCount = getChildCount();
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		...
		switch (action) {
			...
			case MotionEvent.ACTION_MOVE:
				...
				else if (!mPullRefreshing && needAddFooterView() && mContentView.isBottom() && (deltaY < 0 || deltaY > 0 && mHolder.hasFooterPullUp())) {
					sendCancelEvent();
	...
	private void sendCancelEvent() {
		...
		if (!mHasSendCancelEvent) {
			...
			MotionEvent e = MotionEvent.obtain(last.getDownTime(), last.getEventTime() + ViewConfiguration.getLongPressTimeout(), MotionEvent.ACTION_CANCEL, last.getX(), last.getY(),
					last.getMetaState());
	...
	public void setCustomFooterView(View footerView) {
		if (footerView instanceof IFooterCallBack) {
			mFooterView = (XRefreshViewFooter) footerView;
		}
		else {
